...
import { Bound, contains, getCommonBound, inflateBound, transformPointsToNewBound, } from './bound.js';
describe('bound utils', () => {
    ...
    it('intersectLine', () => {
        ...
        expect(bound.intersectLine(Point.from({ x: 0, y: 0 }), Point.from({ x: 10, y: 10 }))).toBeTruthy();
    });
    it('intersectline no intersection', () => {
        const bound = new Bound(0, 0, 10, 10);
        expect(bound.intersectLine(Point.from({ x: 0, y: -1 }), Point.from({ x: 10, y: -10 }))).toBeFalsy();
    });
