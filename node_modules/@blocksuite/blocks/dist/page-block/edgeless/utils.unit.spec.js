...
describe('page-block edgeless utils', () => {
    ...
    it('generateConnectorPath with fixed', () => {
        ...
        const pathWithFixedStart = generateConnectorPath(startRect, endRect, startPoint, endPoint, controllers, ConnectorMode.Orthogonal, 'start');
        expect(pathWithFixedStart).not.toMatchObject(controllers);
        expect(pathWithFixedStart.slice(0, 4)).toMatchObject(controllers.slice(0, 4));
        const pathWithFixedEnd = generateConnectorPath(startRect, endRect, startPoint, endPoint, controllers, ConnectorMode.Orthogonal, 'end');
        expect(pathWithFixedEnd).not.toMatchObject(controllers);
        expect(pathWithFixedEnd.slice(-4)).toMatchObject(controllers.slice(-4));
        const pathStraight = generateConnectorPath(startRect, endRect, startPoint, endPoint, controllers, ConnectorMode.Straight, 'end');
        expect(pathStraight).toMatchObject([startPoint, endPoint]);
        const pathFallback = generateConnectorPath(startRect, endRect, startPoint, startPoint, controllers, ConnectorMode.Straight, 'end');
