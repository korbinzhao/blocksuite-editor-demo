 ...
 * @deprecated
 */
export function normalizeSubpage({ action, pageId, blockId }, workspace, backlinkIndexer) {
    ...
    if (action === 'delete') {
        if (!blockId) {
            // delete page
            if (page) {
                console.warn(`Received delete page event, but page ${pageId} found`, page);
                return;
            }
            // removed subpageId from the parent page when remove page
            const parentMetas = pageMetas.filter(({ subpageIds }) => subpageIds.includes(pageId));
    ...
    // check change
    const diff = diffArray(curSubpageIds, subpageNodes.map(node => node.pageId));
    if (!diff.changed)
        return;
...
//# sourceMappingURL=normalize-subpage.js.map