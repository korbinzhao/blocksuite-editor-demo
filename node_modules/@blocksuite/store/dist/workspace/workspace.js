...
import { WorkspaceMeta } from './meta.js';
...
class Workspace {
        ...
        this._blobStorage = {
            get: async (id) => {
                ...
                return new Promise(res => {
                    this._storages.forEach(storage => storage.crud
                        .get(id)
                        .then(result => {
                        if (result && !found) {
                            found = true;
    ...
    createPage(options = {}) {
        ...
        if (init) {
            pageBlockId = page.addBlock('affine:page', typeof init === 'boolean'
                ? undefined
                : {
    ...
    async importPageSnapshot(json, pageId) {
        ...
        const sanitize = async (props) => {
            ...
            if (props['sys:flavour'] === 'affine:surface' && props['elements']) {
                for (const [, element] of Object.entries(props['elements']
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                )) {
        ...
        const addBlockByProps = async (page, 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        props, parent) => {
    ...
    /** @internal Only for testing */
    exportJSX(blockId, pageId = this.meta.pageMetas.at(0)?.id) {
        assertExists(pageId);
